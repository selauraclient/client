...
#include "shaders_dx.hpp"

namespace sgfx {
    winrt::com_ptr<ID3DBlob> compile_shader_internal(const char* src, const char* target) {
        ...
        if (FAILED(hr)) {
            if (error) spdlog::error("Shader Error: {}", static_cast<char*>(error->GetBufferPointer()));
            return nullptr;
        }
        return blob;
    }
    ...
    bool renderer_d3d11::init(void* sc) {
        ...
        create_shader(shaders::vertex_source, shaders::pixel_source);
    ...
    void renderer_d3d11::render(const draw_data& data) {
        ...
        if (!v_buffer || v_capacity < data.vertices.size()) {
            v_capacity = data.vertices.size() + 1000;
        ...
        for (const auto& cmd : data.commands) {
            ...
            ctx->Draw(cmd.count, vertex_offset);
            vertex_offset += cmd.count;
        }
    }
    ...
    void renderer_d3d11::create_shader(const char* vs_src, const char* ps_src) {
        auto vs_blob = compile_shader_internal(vs_src, "vs_5_0");
        auto ps_blob = compile_shader_internal(ps_src, "ps_5_0");
        ...
        device->CreateInputLayout(layout_desc, 5, vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), layout.put());
    }
}