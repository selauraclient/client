...
namespace sgfx {
    ...
    void draw_rect(float x, float y, float w, float h, glm::vec4 col, glm::vec4 radii) {
        ...
        vertex verts[6] = {
            { {x,     y,     0.0f}, col, {0.0f, 0.0f}, shader_params, radii },
            { {x + w, y,     0.0f}, col, {1.0f, 0.0f}, shader_params, radii },
            { {x + w, y + h, 0.0f}, col, {1.0f, 1.0f}, shader_params, radii },
            { {x,     y,     0.0f}, col, {0.0f, 0.0f}, shader_params, radii },
            { {x + w, y + h, 0.0f}, col, {1.0f, 1.0f}, shader_params, radii },
            { {x,     y + h, 0.0f}, col, {0.0f, 1.0f}, shader_params, radii }
        };

        for (const auto& v : verts) {
            ctx.data.vertices.push_back(v);
        }
    ...
    void draw_image(Resource res, float x, float y, float w, float h, glm::vec4 col, glm::vec4 radii) {
        ...
        if (cache.id == nullptr) {
            ...
            unsigned char* pixels = stbi_load_from_memory(
                (const stbi_uc*)res.data(), (int)res.size(),
                &cache.width, &cache.height, &channels, 4
            );
    ...
    void draw_gif(Resource res, float x, float y, float w, float h, float time, glm::vec4 col, glm::vec4 radii) {
        ...
        if (cache.frames.empty()) {
            ...
            unsigned char* pixels = stbi_load_gif_from_memory(
                (const stbi_uc*)res.data(), (int)res.size(),
                &delays, &x_size, &y_size, &z_size, &channels, 4
            );
    ...
    uint32_t decode_utf8(std::string_view text, size_t& i) {
        ...
        if (c < 0x80) return c;
        if ((c & 0xE0) == 0xC0) {
            uint32_t res = ((c & 0x1F) << 6) | (text[i++] & 0x3F);
            return res;
        }
        if ((c & 0xF0) == 0xE0) {
            uint32_t res = ((c & 0x0F) << 12) | ((text[i++] & 0x3F) << 6) | (text[i++] & 0x3F);
            return res;
        }
        if ((c & 0xF8) == 0xF0) {
            uint32_t res = ((c & 0x07) << 18) | ((text[i++] & 0x3F) << 12) | ((text[i++] & 0x3F) << 6) | (text[i++] & 0x3F);
            return res;
        }
        return 0;
    }
    ...
    void set_font(Resource tex_res, Resource json_res, bool aliased) {
        ...
        for (const auto& g : data_json.glyphs) {
            ...
            if (g.planeBounds) {
                gl.x0 = g.planeBounds->left;
                gl.y0 = g.planeBounds->bottom;
                gl.x1 = g.planeBounds->right;
                gl.y1 = g.planeBounds->top;
            }
            if (g.atlasBounds) {
                gl.u0 = g.atlasBounds->left / f_tw;
                gl.u1 = g.atlasBounds->right / f_tw;
                gl.v0 = 1.0f - (g.atlasBounds->top / f_th);
                gl.v1 = 1.0f - (g.atlasBounds->bottom / f_th);
            }
        }
